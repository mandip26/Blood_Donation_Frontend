...
export const reportProcessingService = {
  ...
  extractDataFromReport: (fileData: string): BloodTestParameter[] => {
    ...
    return [
      ...
      { name: 'RBC', value: 4.9, unit: 'million/μL', normalRange: '4.5-5.9', status: 'normal' },
      { name: 'WBC', value: 7500, unit: '/μL', normalRange: '4500-11000', status: 'normal' },
      { name: 'Platelets', value: 250000, unit: '/μL', normalRange: '150000-450000', status: 'normal' },
      { name: 'Hematocrit', value: 42, unit: '%', normalRange: '38.8-50', status: 'normal' },
      { name: 'MCV', value: 88, unit: 'fL', normalRange: '80-100', status: 'normal' },
      { name: 'MCH', value: 29, unit: 'pg', normalRange: '27-33', status: 'normal' },
      { name: 'MCHC', value: 33, unit: 'g/dL', normalRange: '33-36', status: 'normal' },
      { name: 'RDW', value: 13.4, unit: '%', normalRange: '11.5-14.5', status: 'normal' },
    ];
  },
  ...
  checkDonationEligibility: (report: BloodTestReport): { eligible: boolean; reasons: string[] } => {
    const reasons: string[] = [];
    let eligible = true;
    
    // Check if parameters exist
    if (!report.parameters || report.parameters.length === 0) {
      return { eligible: false, reasons: ['Report does not contain any blood parameters'] };
    }
    ...
    if (hemoglobin) {
      ...
      if (hemoglobin.value < 13.0) {
        eligible = false;
        reasons.push(`Hemoglobin level (${hemoglobin.value} ${hemoglobin.unit}) is below the minimum requirement for donation (13.0 ${hemoglobin.unit})`);
      }
    } else {
    ...
    if (wbc) {
      ...
      if (wbc.value < 4500 || wbc.value > 11000) {
        eligible = false;
        reasons.push(`WBC count (${wbc.value} ${wbc.unit}) is outside the normal range`);
      }
    }
    ...
    if (platelets) {
      ...
      if (platelets.value < 150000) {
        eligible = false;
        reasons.push(`Platelet count (${platelets.value} ${platelets.unit}) is below the minimum requirement`);
      }
    }
...
const reportService = {
  // Upload a new report
  uploadReport: async (reportFile: File, reportData: Partial<BloodTestReport>) => {
    try {
