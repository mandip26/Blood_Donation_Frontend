      ...
      try {
        ...
        try {
          ...
          if (eventsData && eventsData.length > 0) {
            ...
            const formattedEvents: BloodDonationEvent[] = eventsData.map(
              (event: any) => ({
        ...
        try {
          ...
          if (requestsData && requestsData.length > 0) {
            // Map the API response to our interface
            const formattedRequests: BloodRequestCard[] = requestsData.map(
              (request: any) => ({
                id: request._id || request.id,
                bloodType: request.bloodType,
                name: request.name || request.patientName,
                hospital: request.hospital,
                urgency: request.urgency,
                location: request.location,
                postedTime: getTimeAgo(
        ...
        if (user) {
          try {
            ...
            // Try to fetch donor status and next donation date
            // These endpoints might not exist yet, so we handle gracefully
            const donorStatusPromise = donationService
              .getDonorStatus()
              .catch(() => null);
            const nextDonationPromise = donationService
              .getNextDonationDate()
              .catch(() => null);
            ...
            if (donorStatusData) {
              setDonorStats({
                ...
                nextEligible:
                  ...
                  "",
                bloodType: donorStatusData.bloodType || user?.bloodType || "",
              });
              ...
              // Fallback values when endpoints don't exist
              setDonorStats({
                totalDonations: 0,
                lastDonation: "",
                nextEligible: "",
      ...
      <div className="p-4 md:p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="bg-white rounded-xl shadow-sm p-3 md:p-4 flex items-center"
            >
      ...
      {user && (
        <div className="bg-white rounded-xl shadow-sm p-4 md:p-6 mb-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <Link
              to="/dashboard/donate"
              className="text-sm font-medium text-primary-magenta"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
            <div className="p-3 md:p-4 rounded-lg bg-gray-50">
              ...
              <p className="text-lg md:text-xl font-bold">
                {donorStats.bloodType || "Not specified"}
              </p>
            </div>
            ...
            <div className="p-3 md:p-4 rounded-lg bg-gray-50">
              ...
              <p className="text-lg md:text-xl font-bold">
                {donorStatsLoading ? (
                  ...
                  formatDate(donorStats.lastDonation)
                ) : (
                  "None"
                )}
              </p>
            ...
            <div className="p-3 md:p-4 rounded-lg bg-gray-50">
              ...
              <p className="text-lg md:text-xl font-bold">
                {donorStatsLoading ? (
                  ...
                  formatDate(donorStats.nextEligible)
                ) : (
                  "Eligible now"
                )}
              </p>
          ...
          {donorStats.nextEligible && (
            <div className="mt-6">
              ...
              <div className="h-2 bg-gray-200 rounded-full">
                <div
                  className="h-2 bg-[#c6414c] rounded-full"
                  style={{
                    width: `${Math.min(100, ((90 - daysUntilNextDonation()) / 90) * 100)}%`,
                  }}
                ></div>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 md:p-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <Link
              to="/dashboard/events"
              className={`text-sm font-medium text-[#c6414c] ${isMobile ? "hidden" : "block"}`}
            >
              View All
            </Link>
          </div>
          {eventLoading ? (
            <div className="flex items-center justify-center h-40">
              <Loader2 className="h-8 w-8 animate-spin text-[#c6414c]" />
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 md:p-6">
          ...
          {isMobile && bloodRequests.length > 0 && (
            <div className="mt-4 text-center">
              <Link to="/dashboard/recipient">
                <Button variant="outline" size="sm">
                  View All Requests
