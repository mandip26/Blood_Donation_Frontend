...
function RecipientComponent() {
  ...
  const [sorting, setSorting] = useState<{key: keyof BloodRequest | null, direction: 'asc' | 'desc'}>({
    key: null,
    direction: 'desc'
  })
    ...
    const fetchBloodRequests = async () => {
      try {
        ...
        if (requests && requests.length > 0) {
          const formattedRequests: BloodRequest[] = requests.map((request: any) => ({
  ...
  const handleSubmitRequest = async (e: React.FormEvent) => {
    ...
    if (!newRequestForm.hospital.trim()) errors.hospital = 'Hospital name is required'
    if (!newRequestForm.location.trim()) errors.location = 'Location is required'
    if (!newRequestForm.contactNumber.trim()) errors.contactNumber = 'Contact number is required'
    if (!newRequestForm.reason.trim()) errors.reason = 'Reason is required'
    
    // Validate phone number format
    if (newRequestForm.contactNumber && !/^\+?[0-9]{10,15}$/.test(newRequestForm.contactNumber.replace(/\s/g, ''))) {
            ...
            <form onSubmit={handleSubmitRequest} className="p-5">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  {formErrors.bloodType && <p className="text-red-600 text-xs mt-1">{formErrors.bloodType}</p>}
                </div>
                
                <div>
                  <label className="block text-sm font-medium mb-1.5">Hospital*</label>
                  <input
                    type="text"
                  ...
                  />
                  {formErrors.hospital && <p className="text-red-600 text-xs mt-1">{formErrors.hospital}</p>}
                </div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {["low", "medium", "high"].map((level) => (
                      <button
                        type="button"
                        key={level}
                        onClick={() => setNewRequestForm({...newRequestForm, urgency: level as any})}
                ...
                <div>
                  <label className="block text-sm font-medium mb-1.5">Units Required</label>
                  <input
                    type="number"
                    min="1"
                    max="10"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewRequestForm({...newRequestForm, contactNumber: e.target.value})}
                    className={`w-full rounded-md border ${formErrors.contactNumber ? 'border-red-300' : 'border-gray-300'} p-2.5`}
                    placeholder="Phone number"
                  />
                  {formErrors.contactNumber && <p className="text-red-600 text-xs mt-1">{formErrors.contactNumber}</p>}
                </div>
