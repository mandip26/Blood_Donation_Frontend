...
function VisualizationPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const newData = BLOOD_GROUP_DATA.map((group) => ({
        ...
        donations:
          group.donations *
          (timeFrame === "yearly" ? 12 : timeFrame === "weekly" ? 0.25 : 1) *
          (0.9 + Math.random() * 0.2),
        requests:
          group.requests *
          (timeFrame === "yearly" ? 12 : timeFrame === "weekly" ? 0.25 : 1) *
  ...
  const maxValue = Math.max(
    ...data.flatMap((item) => [item.donations, item.requests])
  );
    ...
    <div className="space-y-6 p-4">
      <div className="bg-white rounded-xl p-6 shadow-sm">
        ...
        <p className="text-gray-600">
          Visualize blood donation trends and demands across different blood
          groups.
        </p>
            ...
            <div>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4">
                        <span className="text-sm text-gray-500">
                          {Math.round(group.donations)} /{" "}
                          {Math.round(group.requests)}
                        </span>
              ...
              <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                ...
                <p className="text-sm text-gray-600">
                  For the {timeFrame} period, there{" "}
                  {timeFrame === "monthly" ? "is" : "are"} a total of{" "}
                  <span className="font-medium">
                    {Math.round(
                      data.reduce((sum, item) => sum + item.donations, 0)
                    )}
                  </span>{" "}
                  donations and{" "}
                  <span className="font-medium">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white rounded-xl p-6 shadow-sm">
          ...
          <p className="text-gray-600">
            Blood donation trends have been{" "}
            {timeFrame === "yearly"
              ? "increasing steadily"
              : timeFrame === "monthly"
                ? "fluctuating seasonally"
                : "varying weekly"}{" "}
            with the highest demand for O+ and A+ blood types.
          </p>
          <div className="mt-4">
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm">
          ...
          <div className="mt-4 p-3 bg-yellow-50 rounded-lg">
            <p className="text-yellow-800 text-sm">
              <strong>Note:</strong> Blood types O- and AB- are particularly
              important as O- is a universal donor and AB- is a rare blood type.
            </p>
          </div>
        </div>
      </div>
